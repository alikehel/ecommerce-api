...
export function requestParamsSchema<T extends z.ZodTypeAny>(schema: T): z.ZodEffects<T, ObjectToCamel<z.infer<T>>> {
    return schema.transform((data) => objectToCamel(data) as ObjectToCamel<z.infer<T>>);
}
...
const paramsPaginationSchema = (sortingFields: string[]) =>
    z.object({
        ...
        sorting_field: z.enum(sortingFields as [string, ...string[]]).default("id"),
    });
...
> {
    ...
    return schema
        ...
        .transform(
            (data) => objectToCamel(data) as ObjectToCamel<z.infer<T & ReturnType<typeof paramsPaginationSchema>>>,
        );
}
